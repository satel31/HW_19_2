...
from apps.users.services import send_email, send_email_pswrd
...
class PasswordView(UpdateView):
    ...
    def form_valid(self, form):
        if form.is_valid:
            self.object = form.save()
            new_pswrd = self.object.make_random_password()
            self.object.set_password(new_pswrd)
            self.object.save()
            send_email_pswrd(new_pswrd, self.object.email)
        return super().form_valid(form)
